/*! gridster.js - v0.5.6 - 2015-01-23 - * http://gridster.net/ - Copyright (c) 2015 ducksboard; Licensed MIT */
!function (a, b) {
    ...
    return c.init = function () {
        ...
        var e = this.data;
        return "undefined" == typeof e.left && (e.left = e.x1), "undefined" == typeof e.top && (e.top = e.y1), this.coords.x1 = e.left, this.coords.y1 = e.top, this.coords.x2 = e.left + e.width, this.coords.y2 = e.top + e.height, this.coords.cx = e.left + e.width / 2, this.coords.cy = e.top + e.height / 2, this.coords.width = e.width, this.coords.height = e.height, this.coords.el = c || !1, this
    }, c.update = function (b) {
    ...
    function b(b, d, e) {
        this.options = a.extend(c, e), this.$element = b, this.last_colliders = [], this.last_colliders_coords = [], this.set_colliders(d), this.init()
    }

    var c = {colliders_context: document.body, overlapping_region: "C"};
    b.defaults = c;
    var d = b.prototype;
    ...
    function b(b, d) {
        this.options = a.extend({}, c, d), this.$document = a(document), this.$container = a(b), this.$dragitems = a(this.options.items, this.$container), this.is_dragging = !1, this.player_min_left = 0 + this.options.offset_left, this.id = i(), this.ns = ".gridster-draggable-" + this.id, this.init()
    }
    ...
    var c = {
        ...
        helper: !1,
        remove_helper: !0
    }, d = a(window), e = {x: "left", y: "top"}, f = !!("ontouchstart"in window), g = function (a) {
    ...
    return j.init = function () {
        ...
        this.calculate_dimensions(), this.$container.css("position", "static" === b ? "relative" : b), this.disabled = !1, this.events(), a(window).bind(this.nsEvent("resize"), throttle(a.proxy(this.calculate_dimensions, this), 200))
    ...
    }, j.scroll_in = function (b, c) {
        var f, h = e[b], i = 50, j = 30, k = "x" === b, l = k ? this.window_width : this.window_height, m = k ? a(document).width() : a(document).height(), n = k ? this.$player.width() : this.$player.height(), o = d["scroll" + g(h)](), p = o, q = p + l, r = q - i, s = p + i, t = p + c.pointer[h], u = m - l + n;
        return t >= r && (f = o + j, u > f && (d["scroll" + g(h)](f), this["scroll_offset_" + b] += j)), s >= t && (f = o - j, f > 0 && (d["scroll" + g(h)](f), this["scroll_offset_" + b] -= j)), this
        ...
        this.disable(), this.$container.off(this.ns), this.$document.off(this.ns), a(window).off(this.ns), a.removeData(this.$container, "drag")
    ...
    var d = {
        ...
        collision: {},
        draggable: {items: ".gs-w", distance: 4, ignore_dragging: b.defaults.ignore_dragging.slice(0)},
        resize: {
            enabled: !0,
            axes: ["both"],
            handle_append_to: "",
    ...
    return e.init = function () {
        ...
        return this.$widgets = this.$widgets.add(j), this.register_widget(j), this.add_faux_rows(i.size_y), g && this.set_widget_max_size(j, g), h && this.set_widget_min_size(j, h), this.set_dom_grid_width(), this.set_dom_grid_height(), this.drag_api.set_limits(this.cols * this.min_widget_width), j.fadeIn()
        ...
        b || (b = e.size_x), c || (c = e.size_y), 1 / 0 !== g && (b = Math.min(b, g - f + 1)), c > h && this.add_faux_rows(Math.max(c - h, 0));
        var k = f + b - 1;
        k > this.cols && this.add_faux_cols(k - this.cols);
        var l = {col: j, row: e.row, size_x: b, size_y: c};
        ...
        if (c.col = d.col, c.row = d.row, c.size_x = d.size_x, c.size_y = d.size_y, this.add_to_gridmap(d, b), b.removeClass("player-revert"), b.data("coords").update({
        ...
        for (var d, e = this.gridmap, f = e.length, g = [], h = 1; f > h; h++) {
            d = e[h].length;
            for (var i = 1; d >= i; i++) {
                var j = this.can_move_to({size_x: a, size_y: b}, h, i);
                j && g.push({col: h, row: i, size_y: b, size_x: a})
            }
        }
    ...
    }, e.coords_to_dom = function (a) {
        var b = this.options.widget_base_dimensions, c = this.options.widget_margins;
        return {
            x: (a.col - 1) * (b[0] + 2 * c[0]) + c[0],
            y: (a.row - 1) * (b[1] + 2 * c[1]) + c[1],
            width: a.size_x * b[0] + 2 * (a.size_x - 1) * c[0],
            height: a.size_y * b[1] + 2 * (a.size_y - 1) * c[1]
        }
    }, e.is_clipped_vertically = function (a) {
        var b = this.coords_to_dom(a);
        return b.y + b.height + this.options.widget_margins[1] + 16 > this.$wrapper.height()
    }, e.move_widget_to_closest_available_cell = function (b) {
        for (var c = b.coords().grid, d = c.col, e = a.extend({}, c), f = !1, g = this.get_highest_occupied_row_for_col(e.col) + 2; g > 1 && !f;)g--, f = this.move_widget_to_row_in_col(b, g, e.col);
        if (f)return this;
        for (var h = 1; d - h > 0 && !f;) {
            e.col = d - h;
            for (var g = this.get_highest_occupied_row_for_col(e.col) + 2; g > 1 && !f;)g--, f = this.move_widget_to_row_in_col(b, g, e.col);
            if (f)return this;
            for (e.col = d + h, g = this.get_highest_occupied_row_for_col(e.col) + 2; g > 1 && !f;)g--, f = this.move_widget_to_row_in_col(b, g, e.col);
            if (f)return this;
            h++
        }
        for (; !f && e.col < 100;) {
        ...
        return f && (d.row = f, b.attr("data-row", f), this.$el.trigger("gridster:positionchanged", [d]), e = !0), this.options.avoid_overlapped_widgets && !this.can_move_to({
            size_x: d.size_x,
            size_y: d.size_y
            ...
            "data-sizex": d.size_x,
            "data-sizey": d.size_y
        }), e = !0), b.data("coords", b.coords()), b.data("coords").grid = d, this.add_to_gridmap(d, b), this.options.resize.enabled && this.add_resize_handle(b), e
        ...
        if (this.options.autogrow_cols) {
            ...
            d >= this.cols - 1 && this.options.max_cols >= this.cols + 1 && (this.add_faux_cols(1), this.set_dom_grid_width(this.cols + 1), this.drag_api.set_limits(this.container_width)), this.collision_api.set_colliders(this.faux_grid)
        }
        ...
        }).appendTo(this.$el), this.$resized_widget.addClass("resizing"), this.$vertically_clipped_widgets = [], this.options.resize.start && this.options.resize.start.call(this, b, c, this.$resized_widget), this.$el.trigger("gridster:resizestart")
        ...
        }, this), 300), this.set_dom_grid_width(), this.options.autogrow_cols && this.drag_api.set_limits(this.cols * this.min_widget_width), this.$vertically_clipped_widgets || (this.$vertically_clipped_widgets = []), this.$vertically_clipped_widgets.push(this.$resized_widget);
        for (var d = this.$vertically_clipped_widgets.length, e = 0; d > e; e++) {
            var f = this.$vertically_clipped_widgets[e];
            this.is_clipped_vertically(f.coords().grid) && this.move_widget_to_closest_available_cell(f)
        }
        ...
        if (s = Math.max(Math.min(s, j), k), s = Math.min(u, s), c = j * f + (s - 1) * h * 2, o = Math.min(c, v), min_width = k * f + (s - 1) * h * 2, t = Math.max(Math.min(t, l), m), p = l * g + (t - 1) * i * 2, min_height = m * g + (t - 1) * i * 2, this.resize_dir.right ? t = this.resize_initial_sizey : this.resize_dir.bottom && (s = this.resize_initial_sizex), n) {
            var w = this.resize_initial_col + s - 1;
            n && this.resize_initial_last_col <= w && (this.set_dom_grid_width(Math.max(w + 1, this.cols)), this.cols < w && this.add_faux_cols(w - this.cols))
        }
        var x = {};
        ...
        }, this)), d = b.not(e), {can_go_up: c.sort_by_row_asc(f), can_not_go_up: c.sort_by_row_desc(g)}
    ...
    }, e.is_cells_occupied = function (a) {
        for (var b = a.col, c = a.row, d = b + a.size_x, e = c + a.size_y, f = b; d > f; f++)for (var g = c; e > g; g++)if (this.is_occupied(f, g))return !0;
        return !1
        ...
        var e = b.coords().grid, f = (c - e.row, a.extend({}, e));
        f.col = d, f.row = c;
        var g = !(this.is_cells_occupied(f) || this.is_clipped_vertically(f));
        return g === !1 ? !1 : (this.remove_from_gridmap(e), e.row = c, e.col = d, this.add_to_gridmap(e), b.attr("data-row", c), b.attr("data-col", d), this.$changed = this.$changed.add(b), this)
        ...
        if (-1 === a.inArray(b, f)) {
            ...
            }, this)), h.row = i, this.update_widget_position(h, b), b.attr("data-row", h.row), this.$changed = this.$changed.add(b), f.push(b), this.is_clipped_vertically(h) && this.$vertically_clipped_widgets && -1 === a.inArray(b, this.$vertically_clipped_widgets) && this.$vertically_clipped_widgets.push(b)
        }
        ...
        return this.for_each_column_occupied(d, function (b) {
            e.for_each_widget_below(b, f, function () {
                return e.is_player(this) || -1 !== a.inArray(this, g) ? void 0 : (g = g.add(this), !0)
            })
        }), c.sort_by_row_asc(g)
        ...
        return 1 === c ? !1 : (this.for_each_column_occupied(b, function (a) {
            this.is_widget(a, d);
            return this.is_occupied(a, d) || this.is_player(a, d) || this.is_placeholder_in(a, d) || this.is_player_in(a, d) ? (e = !1, !0) : void 0
        }), e)
    ...
    }, e.get_highest_occupied_row_for_col = function (a) {
        var b, c = this.gridmap, d = c[1].length, e = [0];
        for (b = d - 1; b >= 1; b--)if (this.is_widget(a, b)) {
            e.push(b);
            break
        }
        ...
        return {col: Math.max.apply(Math, e), row: Math.max.apply(Math, d)}
    ...
    }, e.set_dom_grid_height = function () {
        return this.container_height = this.$wrapper.height(), this.$el.css("height", this.wrapper_height), this
        ...
        return b.getElementsByTagName("head")[0].appendChild(c), c.setAttribute("type", "text/css"), c.styleSheet ? c.styleSheet.cssText = a : c.appendChild(document.createTextNode(a)), this.$style_tags = this.$style_tags.add(c), this
        ...
        return this.baseX = (a(window).width() - b) / 2, this.baseY = this.$wrapper.offset().top, a.each(this.faux_grid, a.proxy(function (a, b) {
        ...
        }), this.rows = Math.max(g, this.options.min_rows), this.baseX = (a(window).width() - b) / 2, this.baseY = this.$wrapper.offset().top, this.options.autogenerate_stylesheet && this.generate_stylesheet(), this.generate_faux_grid(this.rows, this.cols)
    }, e.destroy = function (b) {
        return this.$el.removeData("gridster"), a(window).unbind(".gridster"), this.drag_api && this.drag_api.destroy(), this.remove_style_tags(), b && this.$el.remove(), this
    ...
    "function" == typeof define && define.amd ? define(["jquery", "gridster"], b) : a.Gridster = b(a.$ || a.jQuery, a.Gridster)
}(this, function (a, b) {
    var c = b.prototype;
    return c.widgets_in_col = function (a) {
        if (!this.gridmap[a])return !1;
        for (var b = this.gridmap[a].length - 1; b >= 0; b--)if (this.is_widget(a, b) !== !1)return !0;
        return !1
    }, c.widgets_in_row = function (a) {
        for (var b = this.gridmap.length; b >= 1; b--)if (this.is_widget(b, a) !== !1)return !0;
        return !1
    }, c.widgets_in_range = function (b, c, d, e) {
        var f, g, h, i, j = a([]);
        for (f = d; f >= b; f--)for (g = e; g >= c; g--)h = this.is_widget(f, g), h !== !1 && (i = h.data("coords").grid, i.col >= b && i.col <= d && i.row >= c && i.row <= e && (j = j.add(h)));
        return j
    }, c.get_bottom_most_occupied_cell = function () {
        var a = 0, b = 0;
        return this.for_each_cell(function (c, d, e) {
            c && e > a && (a = e, b = d)
        }), {col: b, row: a}
    }, c.get_right_most_occupied_cell = function () {
        var a = 0, b = 0;
        return this.for_each_cell(function (c, d, e) {
            return c ? (a = e, b = d, !1) : void 0
        }), {col: b, row: a}
    }, c.for_each_cell = function (a, b) {
        b || (b = this.gridmap);
        var c = b.length, d = b[1].length;
        a:for (var e = c - 1; e >= 1; e--)for (var f = d - 1; f >= 1; f--) {
            var g = b[e] && b[e][f];
            if (a) {
                if (a.call(this, g, e, f) === !1)break a
            } else;
        }
    }, c.next_position_in_range = function (a, c, d) {
        a || (a = 1), c || (c = 1);
        for (var e, f = this.gridmap, g = f.length, h = [], i = 1; g > i; i++) {
            e = d || f[i].length;
            for (var j = 1; e >= j; j++) {
                var k = this.can_move_to({size_x: a, size_y: c}, i, j, d);
                k && h.push({col: i, row: j, size_y: c, size_x: a})
            }
        }
        return h.length >= 1 ? b.sort_by_col_asc(h)[0] : !1
    }, c.closest_to_right = function (a, b) {
        if (!this.gridmap[a])return !1;
        for (var c = this.gridmap.length - 1, d = a; c >= d; d++)if (this.gridmap[d][b])return {col: d, row: b};
        return !1
    }, c.closest_to_left = function (a, b) {
        this.gridmap.length - 1;
        if (!this.gridmap[a])return !1;
        for (var c = a; c >= 1; c--)if (this.gridmap[c][b])return {col: c, row: b};
        return !1
    }, b
});